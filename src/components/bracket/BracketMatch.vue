<template>
  <div
    class="relative text-[0.8em] flex items-center"
    :class="{ group: index % 2 == 0 && totalMatches > 1 }"
  >
    <div class="flex flex-col w-full">
      <div
        class="px-3 py-1 text-xs text-gray-500 dark:text-gray-400 flex items-center justify-center"
      >
        <input
          type="datetime-local"
...
const canEdit = computed(() => {
  return (
    props.roundIndex === 0 && props.permissions[PERMISSIONS.CAN_SELECT_TEAM]
  );
});
...
const canEditScore = computed(() => {
  return (
    props.match[TEAM_POSITION.ONE].name !== TBD &&
...
const onMatchClick = (event) => {
  ...
  if (
    ...
    tag === "option" ||
    tag === "button" ||
    event.target.closest("input,select,option,button")
  )
    return;
