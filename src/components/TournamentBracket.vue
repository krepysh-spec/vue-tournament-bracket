...
const eliminationStandings = computed(() => {
  if (
    props.format === TOURNAMENT_FORMAT.SWISS ||
    props.format === TOURNAMENT_FORMAT.ROUND_ROBIN
  )
    return [];
...
const standingsData = computed(() => {
  ...
  if (props.format === TOURNAMENT_FORMAT.ROUND_ROBIN)
    return roundRobinStandings.value;
...
const updateUpperMatch = (roundIndex, matchIndex, updatedMatch) => {
  console.log("Updating upper match:", {
    roundIndex,
    matchIndex,
    updatedMatch,
  });
